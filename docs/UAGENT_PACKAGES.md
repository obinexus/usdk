...
| `driver-llm-local` | **Real local inference** via WebLLM (WebAssembly + WebGPU, in-browser, no server) - `SmolLM2-360M-Instruct-q4f16_1-MLC`, opt-in only (`loadModel()` never runs on page load). Labels every response `"[local: <model id>]"`. Browser-only; the WebLLM library itself loads via a runtime CDN `import()`, not an npm dependency. | JS, browser | `contracts`, `capability-llm` | dynamic `import()` for the driver module; a separate, user-triggered CDN `import()` + HTTP model download for the model itself | same | `test/driver-llm-local.test.mjs` + verified live in a real browser with a real question/answer (see `docs/VALIDATION.md`) |
